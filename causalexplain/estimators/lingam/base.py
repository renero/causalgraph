...
from causalexplain.estimators.lingam.bootstrap import BootstrapMixin
from causalexplain.estimators.lingam.utils import predict_adaptive_lasso
...
class _BaseLiNGAM(BootstrapMixin, metaclass=ABCMeta):
    ...
    def get_error_independence_p_values(self, X):
        ...
        for i, j in itertools.combinations(range(n_features), 2):
            ...
            hsic = HSIC()
            hsic.fit(
                np.reshape(E[:, i], [n_samples, 1]), np.reshape(
                    E[:, j], [n_samples, 1])
            )
            p_value = hsic.p_value
